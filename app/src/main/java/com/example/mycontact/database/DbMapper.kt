...
class DbMapper {
    ...
    fun mapContact(contactDbModel: ContactDbModel, tagDbModel: TagDbModel): ContactModel {
        ...
        return with(contactDbModel) { ContactModel(id, name, numphone, isCheckedOff,  isColsefriend, tag,) }
    }
    ...
    fun mapDbContact(contact: ContactModel): ContactDbModel =
        with(contact) {
            ...
            val isColsefriend = isColsefriend
            if (id == NEW_CONTACT_ID)
                ContactDbModel(
                    ...
                    tagId = tag.id,
                    isInTrash = false,
                    isColsefriend = isColsefriend
                )
            else
                ContactDbModel(id, name, content, canBeCheckedOff, isCheckedOff, tag.id, false, isColsefriend)
        }
}